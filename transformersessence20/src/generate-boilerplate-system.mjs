...
class SystemGenerator {
   ...
   *
   * Replace refercnes to 'xavessence20', 'XavEssence20', and 'BOILERPLATE'
  ...
  replaceFileContents() {
    ...
    const replacements = [
      {
        pattern: new RegExp(/game\.xavessence20/g),
        replacement: `game.${this.propName}`
      },
      {
        pattern: new RegExp(/flags\.xavessence20/g),
        replacement: `flags.${this.propName}`
      },
      {
        pattern: 'xavessence20',
   ...
   * Rename files.
   *
   * Rename files that had xavessence20 in their name, such as
   * css/xavessence20.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*xavessence20*.*`).then(files => {
      files.forEach(oldPath => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(oldPath, `${directory}/${file.replaceAll('xavessence20', this.packageName)}`, (err) => {
          if (err) throw err;
        });
      })
    })
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-xavessence20-system.mjs`);
...
inquirer
  ...
  .then((answers) => {
    ...
    // Glob XavEssence20's files so that we can process them.
    glob('*', {ignore: ['node_modules/**'] }).then(files => {
      // Copy all files into the build dir.
      generator.copyFiles(files);
      // Replace xavessence20 name mentions in files.
      generator.replaceFileContents();
      // Rename files that had xavessence20 in their name.
